...
class SessionManager:
    ...
    async def update_session_with_recording(self, upload_response_data: dict):
        """
        Update session with recording URL and metadata from successful upload.
        
        Args:
            upload_response_data: The 'data' portion of successful upload response
            Example: {
        ...
        """
        try:
            # Log the recording information as a structured event
            log_event({
                "type": "recording_uploaded",
                "recording_url": upload_response_data.get("url"),
                "recording_filename": upload_response_data.get("filename"),
                "original_filename": upload_response_data.get("originalName"),
                "file_size": upload_response_data.get("size"),
                "relative_url": upload_response_data.get("relativeUrl"),
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "persona": getattr(self.session, "persona_name", None),
                "room": getattr(getattr(self.session, "room", None), "name", None)
            })
            ...
            if hasattr(self.session, "recording_url"):
                self.session.recording_url = upload_response_data.get("url")
            else:
                setattr(self.session, "recording_url", upload_response_data.get("url"))
            
            logging.info(f"SessionManager: Recording metadata saved - URL: {upload_response_data.get('url')}")
