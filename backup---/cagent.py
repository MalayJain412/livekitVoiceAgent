...
try:
    ...
    from livekit.protocol import AudioCodec, EncodedFileType, EgressStatus
    ...
    class LiveKitAPI: pass
    class RoomCompositeEgressRequest: pass
    class EncodedFileOutput: pass
    class ListEgressRequest: pass
    class AudioCodec: pass
...
async def entrypoint(ctx: JobContext):
    ...
    egress_id = None
    requested_filename_base = None  # Store just the name part, e.g., number-....ogg
    
    lkapi = None  # Define lkapi outside try block for wider scope
    try:
        # -----------------------------------------------------------------
        # --- Start Recording ---
        # -----------------------------------------------------------------
        ...
        if not LIVEKIT_API_AVAILABLE:
            logging.error("Recording skipped: LiveKit API not available")
        else:
            ...
            if not (livekit_url and livekit_api_key and livekit_api_secret):
                ...
                # Use .ogg extension, explicitly set container
                requested_filename_base = f"{ctx.room.name}-{int(time.time())}.ogg"
                filepath_in_container = f"recordings/{requested_filename_base}"

                file_output = EncodedFileOutput(
                    filepath=filepath_in_container,
                    output_format=EncodedFileType.OGG
                )
            ...
            logging.info(f"Requesting recording for room {ctx.room.name}...")
            info = await lkapi.egress.start_room_composite_egress(req)
            egress_id = info.egress_id
            logging.info(f"Recording requested: egress_id={egress_id}, target_file={filepath_in_container}")

    except Exception as e:
        logging.error(f"Recording start error for room {ctx.room.name}: {e}")
        if lkapi:  # Close client if start failed
            await lkapi.aclose()
            lkapi = None  # Prevent trying to use it later
