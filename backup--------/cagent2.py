...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        # --- ENHANCED UPLOAD RECORDING LOGIC (using SessionManager data) ---
        try:
            # Get recording metadata from SessionManager instead of local variables
            recording_metadata = session_manager.get_recording_metadata() if 'session_manager' in locals() else {}
            call_metadata = session_manager.get_call_metadata() if 'session_manager' in locals() else {}
            ...
            if UPLOAD_ENABLED and session_egress_id and session_lkapi and LIVEKIT_API_AVAILABLE:
                ...
                try:
                    ...
                    if DELETE_LOCAL_AFTER_UPLOAD and final_filepath_on_host and (not upload_response or not upload_response.get("success")):
                        logging.warning(f"Attempting to delete local file {final_filepath_on_host} after failed upload...")
                        try:
                            await delete_local_file(final_filepath_on_host)
                        except Exception as del_err:
                            logging.error(f"Failed to delete local file after failed upload: {del_err}")
                         
            elif session_lkapi and LIVEKIT_API_AVAILABLE:
                ...
                try:
                    ...
                    logging.error(f"Error closing API client (no recording): {close_err}")
            
            # Fallback: if SessionManager data not available, use original variables
            elif UPLOAD_ENABLED and 'egress_id' in locals() and 'lkapi' in locals() and LIVEKIT_API_AVAILABLE:
                logging.warning("SessionManager data not available - falling back to local variables for upload")
                # Close the client at minimum
                try:
                    await lkapi.aclose()
                    logging.info("LiveKit API client closed (fallback).")
                except Exception as close_err:
                ...
                logging.info(f"  - session_egress_id: {session_egress_id if 'session_egress_id' in locals() else 'Not found'}")
                logging.info(f"  - session_lkapi: {session_lkapi is not None if 'session_lkapi' in locals() else 'Not found'}")
                logging.info(f"  - LIVEKIT_API_AVAILABLE: {LIVEKIT_API_AVAILABLE}")
                logging.info(f"  - session_manager available: {'session_manager' in locals()}")
                logging.info(f"  - recording_metadata: {recording_metadata if 'recording_metadata' in locals() else 'Not found'}")
                logging.info(f"  - call_metadata: {call_metadata if 'call_metadata' in locals() else 'Not found'}")
                
        except Exception as upload_error:
            logging.error(f"Error in upload logic: {upload_error}", exc_info=True)
            # Ensure API client is closed even if errors occur
            try:
                if 'lkapi' in locals() and lkapi and LIVEKIT_API_AVAILABLE:
                    await lkapi.aclose()
                    logging.info("LiveKit API client closed (error fallback).")
            except Exception:
                pass
