...
# Mobile API integration for metadata
try:
    from mobile_api import get_campaign_metadata_for_call, generate_metadata_filename
    MOBILE_API_AVAILABLE = True
    logging.info("Mobile API integration available for metadata")
except ImportError:
    MOBILE_API_AVAILABLE = False
    logging.info("Mobile API integration not available - using timestamp-based filenames")
        ...
        try:
            if MONGODB_AVAILABLE and _current_session_id:
                try:
                    ...
                    if events:
                        save_conversation_session(events, metadata={"auto_saved": True}, dialed_number=_current_dialed_number)
                except Exception as e:
...
def set_dialed_number(dialed_number: str) -> None:
    """Set the current dialed number for metadata-based file naming"""
...
def get_current_session_id() -> Optional[str]:
    """Alias for get_session_id() for consistency"""
    return _current_session_id

def set_current_session_id(session_id: str) -> None:
    """Alias for set_session_id() for consistency"""
...
def save_conversation_session(items: list, metadata: Optional[dict] = None, dialed_number: Optional[str] = None) -> Optional[str]:
    """Save complete conversation session to MongoDB and/or file with metadata-based naming"""
    ...
    session_data = {
        ...
        "metadata": {
            **(metadata or {}),
            "campaign_metadata": campaign_metadata  # Embed for cron matching
        }
    }
    ...
    # Save to file as backup with metadata-based naming
    session_file = None
    try:
        if campaign_metadata and MOBILE_API_AVAILABLE:
            # Use metadata-based filename for reliable matching
            session_filename = generate_metadata_filename("transcript_session", campaign_metadata, ".json")
            session_file = DEFAULT_DIR / session_filename
        else:
