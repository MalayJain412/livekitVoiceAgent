...
def set_campaign_metadata_for_tools(metadata: dict):
    ...
    global _current_campaign_metadata
    _current_campaign_metadata = metadata
    logging.info(f"Campaign metadata set for tools: {metadata}")
...
def save_lead(lead: dict) -> str:
    """Save lead to MongoDB or JSON file fallback with metadata-based naming."""
    lead_data = {
    ...
    if MONGODB_AVAILABLE:
        try:
            ...
            if lead_id:
                ...
                # Still save to file with metadata for cron processing
                _save_lead_to_file_with_metadata(lead_data)
                return f"mongodb:{lead_id}"
        except Exception as e:
            logging.error(f"Failed to save lead to MongoDB: {e}")
    
    # Save to JSON file with metadata-based naming
    return _save_lead_to_file_with_metadata(lead_data)

def _save_lead_to_file_with_metadata(lead_data: dict) -> str:
    """Save lead to file with metadata-based filename for cron matching"""
    if MOBILE_API_AVAILABLE and _current_campaign_metadata:
        # Use metadata-based filename for reliable matching
        filename = generate_metadata_filename("lead", _current_campaign_metadata, ".json")
        path = os.path.join(LEADS_DIR, filename)
        
        # Add campaign metadata to lead data for verification
        lead_data["campaign_metadata"] = _current_campaign_metadata.copy()
    else:
    ...
    if _current_campaign_metadata:
        logging.info(f"Lead file contains campaign metadata: campaignId={_current_campaign_metadata.get('campaignId')}, voiceAgentId={_current_campaign_metadata.get('voiceAgentId')}")
