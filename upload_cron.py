...
class CentralMetadataUploadCron:
    """Central metadata-based upload cron job using metadata.json files as single source of truth"""
    
    def __init__(self, 
                 metadata_dir: str = "call_metadata",
                 processed_dir: str = "processed_metadata",
                 batch_size: int = 10):
    ...
    def get_pending_metadata_files(self) -> List[Path]:
        """Get metadata.json files ready for upload"""
        if not self.metadata_dir.exists():
            return []

        metadata_files = []
        for mf in self.metadata_dir.glob("metadata_*.json"):
            try:
                with open(mf, 'r', encoding='utf-8') as f:
                    data = json.load(f)

                if data.get('status') == 'ready_for_upload':
                    metadata_files.append(mf)
            except Exception as e:
                logging.error(f"Error reading metadata file {mf}: {e}")
    ...
    def extract_recording_path_from_egress(self, egress_id: str) -> Optional[str]:
        ...
        if not egress_id:
            logging.warning("No egress_id provided for recording extraction")
            return None
        ...
        try:
            ...
            logging.info(f"Looking for egress file: {egress_file}")
            
            if egress_file.exists():
                with open(egress_file, 'r', encoding='utf-8') as f:
                    egress_data = json.load(f)
                
                logging.info(f"Found egress data: {egress_data}")
                ...
                elif 'files' in egress_data and egress_data['files']:
                    # Handle the format we saw: {"files":[{"filename":"recordings/number-_918655066243-1761737754.ogg",...}]}
                    filename = egress_data['files'][0].get('filename')
                    if filename:
                        # Remove 'recordings/' prefix if present to get relative path
                        if filename.startswith('recordings/'):
                            logging.info(f"Extracted recording path: {filename}")
                            return filename
                        else:
                            full_path = f"recordings/{filename}"
                            logging.info(f"Extracted recording path: {full_path}")
                            return full_path
                else:
                    logging.warning(f"No recording path found in egress data for {egress_id}")
            else:
                logging.warning(f"Egress file not found: {egress_file}")
    ...
    def process_metadata_file(self, metadata_file: Path, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            # ALWAYS extract recording path from egress ID (source of truth)
            recording_path = None
            egress_id = campaign_metadata.get('egressId')
            if egress_id:
                ...
                if recording_path:
                    ...
                    # Update metadata with the extracted path
                    metadata['files']['recording'] = recording_path
                else:
                    logging.warning(f"Could not extract recording path from egress {egress_id}")
            else:
                logging.warning(f"No egressId found in campaign metadata for session {session_id}")
            ...
            if conversation_path and Path(conversation_path).exists():
                try:
                    ...
                    logging.error(f"Error loading conversation file {conversation_path}: {e}")
                    return False
            else:
                logging.error(f"Conversation file not found: {conversation_path}")
                return False
            ...
            if lead_path and Path(lead_path).exists():
                try:
                    ...
                    logging.error(f"Error loading lead file {lead_path}: {e}")

            # Generate call ID
            call_id = f"CALL-{session_id}"

            if dry_run:
                logging.info(f"DRY RUN: Would upload call {call_id} with files: recording={recording_path}, conversation={conversation_path}, lead={lead_path}")
                return True

            # Upload complete call data
            logging.info(f"Starting upload for call {call_id} (recording: {recording_path})")
            recording_upload_success = False
            call_data_upload_success = False

            success = upload_complete_call_data_sync(
                campaign_id=campaign_metadata['campaignId'],
                voice_agent_id=campaign_metadata['voiceAgentId'],
                client_id=campaign_metadata['clientId'],
                call_id=call_id,
                caller_phone=metadata['dialed_number'],
                conversation_data=conversation_data,
            ...
            if success:
                call_data_upload_success = True
                self.stats["call_data_uploaded"] += 1
                # If recording path was provided and upload succeeded, recording was also uploaded
                if recording_path:
                    recording_upload_success = True
                    self.stats["recordings_uploaded"] += 1
                    logging.info(f"Recording upload successful for {call_id}")
                else:
                    logging.info(f"Call data upload successful for {call_id} (no recording)")
            else:
                logging.error(f"Upload failed for call {call_id}")            # Consider success if either recording was uploaded OR call data was uploaded
            # (handle case where call data already exists but recording upload succeeded)
            ...
            if overall_success:
                ...
                # Save updated metadata before moving
                ...
                if recording_upload_success and not call_data_upload_success:
                    logging.info(f"Note: Recording uploaded successfully, call data already existed")
                return True
            else:
                ...
                self.stats["failed_uploads"] += 1
                logging.error(f"Failed to upload from metadata: {metadata_file.name}")
                return False
    ...
    def run_scan_and_upload(self, dry_run: bool = False) -> Dict:
        """Main function: scan metadata directory and upload files"""
        
        start_time = datetime.now()
        logging.info(f"Starting central metadata scan and upload (dry_run={dry_run})")
        
        # Reset statistics
        self.stats = {k: 0 for k in self.stats.keys()}
        
        # Get pending metadata files
        metadata_files = self.get_pending_metadata_files()
        
        if not metadata_files:
            logging.info("No pending metadata files found")
            return self.stats
        
        logging.info(f"Processing {min(len(metadata_files), self.batch_size)} metadata files (batch size: {self.batch_size})")
        
        # Process metadata files (limited by batch size)
        processed_count = 0
        for metadata_file in metadata_files:
            ...
            try:
                self.process_metadata_file(metadata_file, dry_run=dry_run)
                processed_count += 1
            except Exception as e:
                logging.error(f"Unexpected error processing {metadata_file}: {e}")
                self.stats["failed_uploads"] += 1
        ...
        # Log summary
        logging.info(f"Central metadata upload completed in {duration:.1f}s:")
        logging.info(f"  Files processed: {self.stats['files_processed']}")
        logging.info(f"  Recordings uploaded: {self.stats['recordings_uploaded']}")
        logging.info(f"  Call data uploaded: {self.stats['call_data_uploaded']}")
        logging.info(f"  Leads uploaded: {self.stats['leads_uploaded']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Upload Cron Script - Directory or Central Metadata Based')
    ...
    parser.add_argument('--use-central-metadata', action='store_true',
                       help='Use central metadata.json approach instead of directory scanning')
    parser.add_argument('--conversations-dir', default='conversations',
                       help='Path to conversations directory (for directory scanning mode)')
    parser.add_argument('--recordings-dir', default='recordings', 
                       help='Path to recordings directory (for directory scanning mode)')
    parser.add_argument('--leads-dir', default='leads',
                       help='Path to leads directory (for directory scanning mode)')
    parser.add_argument('--metadata-dir', default='call_metadata',
                       help='Path to metadata directory (for central metadata mode)')
    ...
    try:
        if args.use_central_metadata:
            # Use new central metadata approach
            logging.info("Using Central Metadata Upload Cron")
            cron = CentralMetadataUploadCron(
                metadata_dir=args.metadata_dir,
                batch_size=args.batch_size
            )
        else:
            # Use traditional directory scanning approach
            logging.info("Using Directory-Based Upload Cron")
